...
export const CommunityChat: React.FC = () => {
    ...
    const requestLocation = React.useCallback(() => {
        ...
        addToast({ type: 'info', message: 'Requesting location for Maps mode...' });
        navigator.geolocation.getCurrentPosition(
            (position) => {
                setUserLocation({ latitude: position.coords.latitude, longitude: position.coords.longitude });
                addToast({ type: 'success', message: 'Location acquired.' });
            },
            () => addToast({ type: 'error', message: 'Unable to retrieve location.' }),
            { timeout: 10000, enableHighAccuracy: false }
        );
    }, [addToast]);

    React.useEffect(() => {
        if (chatMode === 'maps' && !userLocation) requestLocation();
    ...
    if (messages.length >= MAX_MESSAGES) { addToast({ type: 'error', message: `Maximum ${MAX_MESSAGES} messages reached. Clear the chat to continue.` }); return; }
        ...
        try {
            ...
            const contextMessages = contextMsg
              ? [...historyForApi, userMsg, { role: 'user' as const, text: contextMsg }]
    ...
    return (
        <Card className="h-full flex flex-col">
                               ...
                               <div className="mt-3 pt-2 border-t border-slate-200/50">
                                   ...
                                   <ol className="text-xs text-slate-500 list-decimal list-inside space-y-1">
                                       {msg.sources.map((source, i) => {
                                          ...
                                          return <li key={i}><a href={link.uri} target="_blank" rel="noopener noreferrer" className="underline hover:text-slate-700">{link.title || 'Source'}</a></li>;
                                       })}
                                   </ol>
                               </div>
                           )}
            ...
            <div className="p-4 border-t border-slate-200 bg-white space-y-3">
                ...
                <p className="text-xs text-slate-400 text-center">{messages.length - 1} {t('chat.messages')} · {MAX_MESSAGES} {t('chat.max')}</p>
            </div>
        </Card>
    );
};
